...
static void LoadCableCarSprites(void)
{
    ...
    switch (gSpecialVar_0x8004)
    {
        case 0:
        default:
            spriteId = AddPseudoEventObject(playerGraphicsIds[gSaveBlock2Ptr->playerGender], sub_8150948, 200, 73, 102, FALSE); // Don't filter the sprite
        ...
        case 1:
            ...
            spriteId = AddPseudoEventObject(playerGraphicsIds[gSaveBlock2Ptr->playerGender], sub_8150948, 128, 39, 102, FALSE); // Don't filter the sprite
            if (spriteId != MAX_SPRITES)
            {
                gSprites[spriteId].oam.priority = 2;
                gSprites[spriteId].pos2.x = 8;
    ...
    if ((rval % 64) == 0)
    {
        spriteId = AddPseudoEventObject(hikerGraphicsIds[rval % 3], callbacks[gSpecialVar_0x8004], hikerCoords[gSpecialVar_0x8004][0], hikerCoords[gSpecialVar_0x8004][1], 0x6a, FALSE); // Don't filter the sprite
        if (spriteId != MAX_SPRITES)
        {
            gSprites[spriteId].oam.priority = 2;
