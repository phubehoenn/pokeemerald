...
u8 UpdateWeatherPal(void)
{
	...
	if (gWeatherPtr->currWeather == WEATHER_SANDSTORM)
		LoadPaletteWithDayNightFilter(gSandstormWeatherPalette, 16 * index + 0x100, 1, 1, FALSE);
	else if (gWeatherPtr->currWeather == WEATHER_CLOUDS)
		LoadPaletteWithDayNightFilter(gCloudsWeatherPalette, 16 * index + 0x100, 1, 1, FALSE);
	else
		LoadPaletteWithDayNightFilter(gUnknown_083970E8, 16 * index + 0x100, 1, 1, FALSE);
}
