...
void apply_map_tileset_palette(struct Tileset const *tileset, u16 destOffset, u16 size)
{
    ...
    if (tileset)
    {
        ...
        {
			LoadPaletteWithDayNightFilter(((u16*)tileset->palettes), destOffset, size, 1, TRUE); //map is being initialized here, so TRUE
			LoadPalette(&black, destOffset, 2);
        }
        else if (tileset->isSecondary == TRUE)
        {
			LoadPaletteWithDayNightFilter(((u16*)tileset->palettes) + (NUM_PALS_IN_PRIMARY * 16), destOffset, size, 1, TRUE); //map is being initialized here, so TRUE
        }
        else
        {
